...
public class UserService {
	...
	// 2-add new users
	public User addNewUser(String username_input, String pwd,String cardName_input, String card_input, String exp_input, int sec_input) {
		User newu = new User();
		if (emailValid(username_input)) {
			newu.setUsername(username_input);
			BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
			String hp = encoder.encode(pwd);
			// System.out.println(hp+" :"+hp.length());//
			newu.setPassword(hp);
			newu.setAccountname(cardName_input);
			newu.setAccountnum(card_input);
			newu.setExpiredate(exp_input);
			newu.setSn(sec_input);
	...
	public boolean emailVerify(String username, String key) {
		...
		User user = null;
		if (flag) {
			String authority = "ROLE_USER";// set authority for users
